    ...
    <nav class="fixed w-full z-40 bg-ramadan-dark/90 backdrop-blur-md border-b border-ramadan-gold/20">
      <div class="container mx-auto px-4 py-3">
        <div class="flex justify-between items-center">
          ...
          <div class="hidden xl:flex gap-1 text-sm font-medium items-center overflow-x-auto no-scrollbar">
...
function generateDesktopLinks(active) {
  const links = [
    ...
    { id: 'qibla', href: 'qibla.html', text: 'القبلة' },
...
function generateMobileLinks(active) {
  const links = [
    ...
    { id: 'qibla', href: 'qibla.html', text: 'اتجاه القبلة' },
    { id: 'moshaf', href: 'moshaf.html', text: 'تحميل المصحف' },
  ];
  ...
  return links.map(link => {
    const isActive = active === link.id;
    const classes = isActive
      ? 'text-ramadan-gold border-r-4 border-ramadan-gold bg-gradient-to-l from-ramadan-gold/10 to-transparent pl-4'
      : 'text-gray-300 border-r-4 border-transparent hover:text-white hover:bg-white/5 pl-4';

    return `<a href="${link.href}" class="${classes} block py-3 pr-4 transition-all text-base font-medium rounded-l-lg">
      ${link.text}
    </a>`;
  }).join('');
}
