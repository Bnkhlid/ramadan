...
async function loadContentData() {
    ...
    try {
        ...
        const hadithRes = await fetch('data/bukhari.json');
        const hadithData = await hadithRes.json();
        // Flatten hadith structure
        hadithData.forEach(book => {
            if (book.hadiths) {
                book.hadiths.forEach(h => hadithList.push({ text: h.body, source: "صحيح البخاري" }));
            }
        });
    } catch (e) {
